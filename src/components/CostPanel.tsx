...
type Props = {
  ...
  /** M26: tekst na przycisku obok ikony $ (np. wartość kosztu "$0.0069") */
  label?: string;
  /** M26: własny tooltip przycisku (nadpisuje domyślny) */
  hint?: string;
};
  ...
  return (
    <div ref={panelRef} className="relative inline-block">
      ...
      <button
        ...
        title={hint ?? (detectionId ? 'Koszty OCR tej detekcji' : 'Koszty API dla tego zdjęcia')}
              ...
              {filteredVision.map((vr) => (
                ...
                >
                  ...
                  <span className="flex-1 truncate text-[11px] text-gray-600">
                    Vision · {formatDate(vr.created_at)} · {formatLatency(vr.latency_ms)}
                  </span>
                  <span className="flex-shrink-0 text-[11px] font-semibold text-gray-800">
                    {formatCost(vr.cost_usd)}
                  </span>
                </div>
              ))}
            ...
            <div className="border-t border-gray-100 px-3 py-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-gray-500">
                  {filteredCount} {filteredCount === 1 ? 'wywołanie' : 'wywołania/ń'}
                </span>
                <span className="text-xs font-bold text-gray-800">
                  Suma: {formatCost(filteredTotal)}
                </span>
              </div>
            </div>
          )}
