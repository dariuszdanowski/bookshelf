...
const RUN_ID = '00000000-0000-4000-8000-000000000090';
...
const latestRunRow: VisionRunRow = {
  id: RUN_ID,
...
describe('GET /api/photos/[id]', () => {
  ...
  it('returns photo + empty detections when no succeeded vision_run exists', async () => {
    ...
    const json = (await res.json()) as {
      data: { photo: { id: string; status: string }; detections: unknown[]; vision_run: null };
    };
    expect(json.data.photo.id).toBe(PHOTO_ID);
    expect(json.data.photo.status).toBe('uploaded');
    expect(json.data.detections).toHaveLength(0);
    expect(json.data.vision_run).toBeNull();
  });
  ...
  it('returns photo + detections + vision_run metadata for processed photo', async () => {
    ...
    const json = (await res.json()) as {
      data: {
        ...
        vision_run: { id: string; model: string | null } | null;
      };
    };
    ...
    expect(json.data.vision_run?.id).toBe(RUN_ID);
    expect(json.data.vision_run?.model).toBe('claude-sonnet-4-6');
