...
vi.mock('../../../../../src/lib/books/openLibrary', () => ({ searchOpenLibrary: vi.fn(), searchOpenLibraryByTitle: vi.fn() }));
...
function makeContext(opts: {
  ...
  const updateSingle = vi.fn().mockResolvedValue(
    opts.updateError
      ...
      : { data: { id: BOOK_ID, title: 'Nowy', authors: ['A'], cover_url: 'https://c.jpg', cover_source: 'auto' }, error: null }
  );
...
const GB_CANDIDATE = {
  source: 'google_books' as const, externalId: 'gb-1', title: 'Solaris', authors: ['Stanisław Lem'],
  isbn10: null, isbn13: '9780156027601', publisher: 'Harvest', publishedYear: 1961, coverUrl: 'https://gb/cover.jpg',
};
...
describe('POST /api/books/[id]/identify', () => {
  it('401 gdy brak użytkownika', async () => {
    const ctx = makeContext({ body: { mode: 'search', title: 'X' } }) as { locals: { user: unknown } };
  ...
  it('404 gdy książka nie istnieje', async () => {
    const res = await POST(makeContext({ body: { mode: 'search', title: 'Solaris' }, bookExists: false }));
    expect(res.status).toBe(404);
  });

  it('search: zwraca kandydatów (GB)', async () => {
    vi.mocked(searchGoogleBooks).mockResolvedValue({ ok: true, candidates: [GB_CANDIDATE] });
    const res = await POST(makeContext({ body: { mode: 'search', title: 'Solaris', author: 'Lem' } }));
  ...
  it('apply: aktualizuje książkę i zwraca applied', async () => {
    const res = await POST(makeContext({
      body: { mode: 'apply', candidate: { title: 'Solaris', authors: ['Lem'], isbn13: '9780156027601', coverUrl: 'https://gb/cover.jpg', source: 'google_books', externalId: 'gb-1' } },
    }));
    expect(res.status).toBe(200);
    const json = (await res.json()) as ApiJson;
  ...
  it('apply: 400 przy duplikacie ISBN (23505)', async () => {
    const res = await POST(makeContext({
      body: { mode: 'apply', candidate: { title: 'Solaris', authors: ['Lem'], isbn13: '9780156027601' } },
  ...
  it('apply: 400 gdy candidate bez tytułu', async () => {
    const res = await POST(makeContext({ body: { mode: 'apply', candidate: { authors: ['Lem'] } } }));
    expect(res.status).toBe(400);
  });
});
