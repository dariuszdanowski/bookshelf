...
const shelvesBody = {
  data: {
    shelves: [
      { id: SHELF_A, name: 'Salon', location: null, position_index: 0, is_system: false, book_count: 1, created_at: '2026-01-01T00:00:00Z' },
      { id: SHELF_B, name: 'Sypialnia', location: null, position_index: 1, is_system: false, book_count: 0, created_at: '2026-01-01T00:00:00Z' },
    ],
  },
};
...
test.describe('S-07 — move-book golden path (mock)', () => {
  ...
  test('picker przenosi książkę → POST move + optimistic badge na docelową półkę', async ({ page }) => {
    await page.route(`**/api/books/${BOOK_1}/move`, (route) => {
      if (route.request().method() === 'POST') {
        void route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { book_id: BOOK_1, shelf_id: SHELF_B } }) });
      } else {
        void route.continue();
      }
    });
    ...
    const moveReq = page.waitForRequest((r) => r.url().includes(`/api/books/${BOOK_1}/move`) && r.method() === 'POST');
  ...
  test('błąd move (500) → rollback badge do „Salon"', async ({ page }) => {
    await page.route(`**/api/books/${BOOK_1}/move`, (route) => {
      if (route.request().method() === 'POST') {
        void route.fulfill({ status: 500, contentType: 'application/json', body: JSON.stringify({ error: { code: 'INTERNAL_ERROR', message: 'fail' } }) });
      } else {
        void route.continue();
      }
    });
    ...
    await page.waitForResponse((r) => r.url().includes('/api/shelves') && r.request().method() === 'GET');
    await page.getByTestId('search-input').fill('solaris');
    await expect(page.getByTestId(`shelf-badge-${BOOK_1}`)).toHaveText('Salon', { timeout: 10_000 });
