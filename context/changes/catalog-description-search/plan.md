...
- `identify` (`books/[id]/identify.ts`, tryb `apply`) już dziś nadpisuje metadane książki z kandydata — dodanie opisu do tej ścieżki daje **ręczny per-book backfill za darmo**, bez bulk re-fetch.
...
- Test integracyjny zielony: `npm run test:integration`
...
#### 3. Persist kandydatów w pipeline match

**File**: `src/lib/matching/` (miejsce INSERT do `book_candidates` — `findCandidates`/`match`)

**Intent**: Dopisać `description` do insertu kandydatów, żeby ścieżka confirm (czytająca z DB) miała skąd wziąć opis.

**Contract**: INSERT `book_candidates` + kolumna `description`; bez zmian kształtu odpowiedzi poza dodatkowym polem w DTO detekcji/kandydata (nullable — backward compatible).

#### 4. Ścieżki tworzenia/aktualizacji książki

**File**: `src/lib/books/confirm.ts`, `src/pages/api/detections/[id]/confirm.ts`, `src/pages/api/photos/[id]/confirm-batch.ts`, `src/pages/api/books/[id]/identify.ts`, `src/pages/api/books/index.ts` (+ `src/lib/books/schema.ts` `AddPurchaseSchema`, `src/components/BookModal.tsx`)

**Intent**: Propagacja `candidate.description` → `books.description` we wszystkich czterech ścieżkach. W manualnym POST opis przychodzi WYŁĄCZNIE z wybranego kandydata (BookModal przekazuje w body; brak pola edycji w UI — out of scope).

**Contract**: `ConfirmBookInput + description: string | null`; INSERT w `confirm.ts` + kolumna; UPDATE w `identify.ts` (tryb apply) + kolumna — to jest per-book ręczny backfill; `AddPurchaseSchema` + `description: z.string().max(2000).nullable().optional()`; BookModal dołącza `description` z prefillu kandydata do payloadu POST (stan ukryty, bez kontrolki).
...
- (post-merge, user-only) identify na starej książce bez opisu → opis uzupełniony
...
3. Identify na książce sprzed migracji → opis uzupełniony, fraza znajdowalna
...
- [ ] 2.1 Lint + typecheck przechodzą (`npm run lint`, `npm run typecheck`)
- [ ] 2.2 Pełna suita unit zielona z nowymi testami (`npm run test`)
- [ ] 2.3 Test integracyjny nadal zielony (`npm run test:integration`)
- [ ] 2.4 Pełne E2E bez regresji (`npx playwright test`)

#### Manual

- [ ] 2.5 (post-merge, user-only) confirm z GB → opis w Studio → fraza z opisu znajduje książkę
- [ ] 2.6 (post-merge, user-only) identify uzupełnia opis starej książki
