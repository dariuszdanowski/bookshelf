...
## Stale Vite optimized-deps cache → island nie hydratuje (mylący „wieczne Ładowanie")

- **Context**: Lokalny dev (`npm run dev`, Astro + `@astrojs/cloudflare` + Vite) z React islands (`client:load`/`client:visible`) importującymi zależności npm (np. `PhotoUploader` → `supabase.browser` → `@supabase/ssr`). Wyzwalane przez wiele operacji gita (branch-switch, merge) przy **działającym** dev-serverze. Dotyczy też e2e Playwright reużywającego tego dev-servera (`reuseExistingServer`).
- **Problem**: 2026-06-03: po dniu branch-switchy/merge `/upload` zawisło na „Ładowanie półek..." — wyglądało jak regresja zmiany albo bug `/api/shelves`. Realna przyczyna: Vite re-optymalizuje deps i zmienia hash `?v=<hash>`; otwarta karta/dev-server trzyma stary → **404 na `/node_modules/.vite/deps/<dep>.js?v=<hash>`** → dynamiczny import komponentu pada (`[astro-island] Error hydrating <C>.tsx: Failed to fetch dynamically imported module`) → island **nie hydratuje** → `useEffect` nigdy nie rusza → **zero requestów `/api/`** → strona zostaje na stanie początkowym. E2E `/upload` pada w `setupPage` (brak `shelf-select`), fałszywie wskazując na świeżą zmianę (stash-test pre-change padał identycznie). Diagnostyk pokazał `0` requestów `/api/` zanim konsola ujawniła 404 + astro-island error.
- **Rule**: Gdy island (`client:load`) nie hydratuje, a strona wisi na stanie początkowym (zero requestów `/api/`), NAJPIERW sprawdź konsolę: `404` na `/node_modules/.vite/deps/*?v=<hash>` + `[astro-island] Error hydrating ... Failed to fetch dynamically imported module` ⇒ **stale Vite cache, nie bug kodu**. Fix: stop dev server → `Remove-Item -Recurse -Force node_modules\.vite` → `npm run dev` → twardy refresh (Ctrl+Shift+R). NIE dotyczy produkcji (prod = Rollup build, bez `.vite/deps`). Nie diagnozuj jako regresji zmiany ani bugu endpointu, dopóki nie wykluczysz dev-cache; e2e padające w `setupPage` po branch-switchach traktuj jak podejrzenie tego samego — zrestartuj dev server przed werdyktem „regresja".
- **Applies to**: implement, impl-review

## Branchuj od świeżo zmergowanego main — równoległe archiwa konfliktują na roadmap.md ## Done

- **Context**: Każdy slice w branch-per-change, gdy archiwizujesz przez `/10x-archive`. Skill dopisuje wpis do `context/foundation/roadmap.md` sekcji `## Done` (zawsze w to samo miejsce — przed placeholderem), flipuje `Status` w at-a-glance + body, bumpuje frontmatter `updated`. Dotyczy sytuacji, gdy dwa branche archiwizują slice'y zanim poprzedni PR jest zmergowany do main.
- **Problem**: 2026-06-03: S-30 (PR #34) branchowany od main **sprzed** merge'a #33 (S-35). Oba archive dopisały bullet do `## Done` w tym samym miejscu → przy próbie merge PR #34 „ma konflikty". (Status w at-a-glance/body **nie** konfliktuje — różne linie, git 3-way bierze `theirs` gdy `ours==base`; konfliktuje tylko append do `## Done`.) Konflikt trywialny, ale blokuje PR i wymaga ręcznej interwencji.
- **Rule**: Przed `git switch -c change/<id>` na nowy slice **ZAWSZE** `git switch main && git pull` — branch od świeżo zmergowanego main wciąga poprzednie archiwa i nie złapie konfliktu na `## Done`. Gdy konflikt już jest: **merge** `origin/main` do brancha (NIE rebase — `git push --force` jest w deny-liście), rozwiąż `## Done` zachowując **OBA** wpisy (kolejność = chronologia merge'y), `git push` merge commit → PR wraca do `MERGEABLE` bez force-push.
- **Applies to**: implement, archive

## Auto-match i ręczny rematch muszą używać tej samej funkcji wyszukiwania kandydatów

- **Context**: Endpointy `POST /api/photos/[id]/match` (auto-match po vision) i `POST /api/detections/[id]/rematch` (ręczne ponowne wyszukiwanie) — oba szukają kandydatów książki po tytule/autorze, ale używają różnych funkcji.
