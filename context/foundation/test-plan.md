...
|---|------------|-----------------|---------------|------------|--------|----------------|
| 1 | RLS isolation gate w CI | Dowód izolacji per-user wykonuje się automatycznie, nie tylko ręcznie post-merge (podpiąć `test:integration` do istniejącego joba e2e — lokalna Supabase już stoi) | #1, #4 | integration | not started | — |
