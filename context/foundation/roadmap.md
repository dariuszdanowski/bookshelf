...
| S-26  | admin-panel                   | panel administracyjny: lista użytkowników, flaga AI-enabled (domyślnie false — admin włącza), impersonacja (zaloguj się jako user), usunięcie konta (półki/książki przechodzą do admina), przeniesienie półki między użytkownikami | S-01 | NFR (admin ops) | done     |
...
| S-43  | vision-identity-first         | przeorientowanie pipeline'u z „ciasny bbox per książka" na ROZPOZNANIE jako cel: model zwraca listę `{kind, title, author, confidence}` (książki/gry) bez wymuszania współrzędnych; główny flow = karty „potwierdź" z kandydatami match + miniaturą; lokalizacja zredukowana do kolejności + przybliżonego markera; rysowanie/edycja bbox tylko jako narzędzie doszczegółowiania gdy model nie rozpozna pozycji (reuse `e2aa2ed`). KPI: title-recall + precyzja + czas review (nie IoU). | S-40, S-04, S-05, S-18 | FR-010–014 (rozpoznanie) | done     |
| S-44  | e2e-flake-stabilization       | utwardzenie flaky E2E: pełny `npm run test:e2e` przechodzi deterministycznie. Znane flaki (zielone w izolacji, padają w pełnym przebiegu na shared-session storageState): `shelves.spec` (create → czekanie na shelf-item-name), `account.spec` (UserMenu → user-menu-account po kliknięciu triggera), `dark-mode-contrast.spec` (hover „Edytuj"). Przyczyna: timing/hydracja wysp + współdzielona sesja (1 signup/run). Kierunek: jawne `waitFor` hydration/menu, twardsze lokatory, ew. izolacja sesji lub celowane retry — NIE globalny retry maskujący. | — | NFR (rzetelność CI/testów) | done |
| S-45  | proposal-dedup-within-photo   | ostrzeżenie gdy dodana/proponowana książka duplikuje INNĄ propozycję na tym samym zdjęciu (dziś dedup tylko vs potwierdzony katalog `books`, nie vs pending detekcje) — miękki flag „Już rozpoznane jako #N na tym zdjęciu", bez blokady (możliwe 2 fizyczne egzemplarze); rozszerza `checkCatalogDuplicate`/rematch o rodzeństwo-detekcje danego `photo_id`; łapie też podwójną detekcję tego samego grzbietu przez vision | S-04, S-05, S-43 | FR-015–018 (jakość matchingu) | proposed |
| S-46  | user-bug-report               | użytkownik może zgłosić błąd/sugestię przez formularz: podsumowanie + opis + opcjonalne załączniki (zrzuty ekranu); formularz auto-zbiera kontekst techniczny (przeglądarka, OS, URL, viewport, aktywny provider BYOK) + dane zgłaszającego (email, display_name) + dane serwerowe (Worker version z /api/health); raport trafia do GitHub Issues (github.com/dariuszdanowski/bookshelf) jako preferowana destynacja (GitHub API, PAT `issues:write`); fallback: Jira / email / tabela `feedback` w Supabase | S-01, S-11 | NFR (supportability) | proposed |
...
| S-50  | ai-book-resolution-fallback   | Fallback identyfikacji książki przez AI z web search (Claude `web_search` tool, klucz usera — BYOK), uruchamiany WYŁĄCZNIE dla detekcji, dla których strukturalna kaskada (GB/OL/BN) nie znalazła kandydata ≥ MATCH_MID — nie dla każdej detekcji na zdjęciu (koszt liniowy od liczby trudnych przypadków, nie od rozmiaru półki). Zastępuje/unieważnia wcześniejszy pomysł „polish-grammatical-variants" (analizator morfologiczny) — web search naturalnie radzi sobie z odmianą/literówkami OCR bez budowania własnego stemmera. Wykryte przy manualnym teście #153 (2026-07-12): OCR „Złodziej książek" (l. pojedyncza) zamiast „Złodzieje książek" (l. mnoga) — BN zwraca zupełnie inne wyniki dla obu zapytań (keyword search po ich stronie, nie lokalny fuzzy match); word-level fallback (S-48) nie pomaga, bo wymaga obecności autora. Koszt: ~$0.02–0.04 za rozwiązaną książkę (search $0.01/wyszukanie + tokeny), sprawdzone w docs Anthropic (2026-07-12) — porównywalne z pojedynczym vision callem. Świadomie NIE wpinać web search do oryginalnego promptu vision (per-zdjęcie) — przy 30+ grzbietach na zdjęciu koszt mnożyłby się drastycznie i miesza fazy detekcja/matching, które architektura celowo trzyma osobno (idempotencja retry). Otwarte przy `/10x-plan`: batch (kilka nierozwiązanych detekcji w jednym wywołaniu — mniej round-tripów, trudniejsza obsługa częściowego niepowodzenia) vs pojedyncze wywołania per detekcja (prostsze, spójne z istniejącym wzorcem `rematch.ts`); schema Zod wyniku (title/author/isbn/confidence + "not_found"); guardrail kosztowy (limit wywołań per zdjęcie/dzień, jak `daily_vision_budget_usd`). | S-04, S-33 (BYOK) | FR-015–018 (jakość matchingu) | done     |
...
| S-43       | vision-identity-first        | Pivot pipeline'u na identity-first (rozpoznanie > lokalizacja) — wynik S-40 | done | **DUŻE — przez `/10x-plan` (pełny cykl, dotyka prompt+schema+API+UI).** Prompt identity-only (bez bbox; `{kind,title,author,confidence}`, książki+gry); `bbox` → optional w `schema.ts`; review = lista kart „potwierdź" z kandydatami match (overlay/numer jako kotwica pomocnicza, nie ciasny box); bbox-editor (`e2aa2ed`) przepięty na doszczegółowianie gdy brak rozpoznania. KPI z IoU → title-recall+precyzja+czas review. Dowód kierunku: `bbox-identity-test.mjs` (identity ≥ v6 recall, 30–46% taniej). Otwarte: „inne narzędzie" do geometrii (CV grzbietów / Gemini grounding) tylko warunkowo, osobny slice. |
| S-44       | e2e-flake-stabilization      | Utwardzenie flaky E2E (rzetelność testów/CI) | yes | Pełny `npm run test:e2e` ma przechodzić deterministycznie. 3 znane flaki (zielone w izolacji, padają w pełnym przebiegu — timing/hydracja wysp + shared-session storageState 1 signup/run): `shelves.spec` (create→shelf-item-name), `account.spec` (UserMenu→user-menu-account), `dark-mode-contrast.spec` (hover „Edytuj"). Kierunek: jawne `waitFor` na hydrację/otwarcie menu + twardsze lokatory; rozważyć izolację sesji lub celowany retry per-test — NIE globalny retry maskujący prawdziwe regresje. Kontekst: pamięć `ci-e2e-jwt-iat-skew-flake`, `workerd-dist-lock-build-e2e`. Wykryte przy PR #87 (S-40). |
| S-45       | proposal-dedup-within-photo  | Ostrzeżenie „już rozpoznane jako #N" gdy propozycja duplikuje inną detekcję na tym samym zdjęciu | no | Wykryte przy manual-verify S-43 (2026-06-11): „Dodaj pominiętą książkę" dla pozycji już rozpoznanej (np. #1 „Pożeracze Książek") nie ostrzega — dedup (`checkCatalogDuplicate`, `rematch.ts:213`) sprawdza wyłącznie potwierdzony katalog `books`, a #1 nie była jeszcze zaakceptowana. Kierunek: miękki flag (NIE blokada — możliwe 2 egzemplarze) gdy ISBN/fuzzy-title pokrywa się z rodzeństwem-detekcją `photo_id`; łapie też podwójną detekcję grzbietu przez vision. |
| S-46       | user-bug-report              | Formularz zgłaszania błędów/sugestii przez zalogowanego użytkownika | no | Przycisk „Zgłoś problem" (stały w headerze lub floating action). Formularz: podsumowanie (required), opis (optional), załączniki (zrzuty ekranu, opcjonalne). Auto-zebrany kontekst **kliencki**: URL strony, userAgent, viewport, motyw (dark/light), aktywny BYOK provider (nazwa, nie klucz!), ostatnie błędy z `window.onerror`/`console.error` — przechwycone przez ring-buffer. Auto-zebrany kontekst **serwerowy**: wersja Worker z `/api/health` (version+timestamp), Supabase project ref (PUBLIC_SUPABASE_URL — prefix), środowisko (prod/dev). Dane zgłaszającego: `user_id`, email, display_name, data rejestracji konta, liczba półek/książek (z DB). Zapis: tabela `feedback` w Supabase (user_id, summary, description, attachments_urls[], client_context jsonb, server_context jsonb, created_at) + Storage bucket `feedback-attachments/` + powiadomienie email do admina (Cloudflare Email Routing lub Resend) albo automatyczne zgłoszenie Jira (Atlassian API). Decyzja destynacji do podjęcia przy `/10x-plan` (GitHub Issues https://github.com/dariuszdanowski/bookshelf/issues — preferowane, tworzy issue przez GitHub API z wypełnionym body; Jira vs email vs tylko DB + panel admin jako fallback). GitHub Issues: issue body = markdown z sekcjami Opis / Kontekst techniczny / Zgłaszający; label `bug` lub `enhancement` wybierany przez usera; wymaga `GITHUB_ISSUE_TOKEN` (fine-grained PAT: `issues:write` na repo). Prereqs: S-01 (auth), S-11 (health), S-26 (panel admin dla podglądu). |
...
- **S-40: jakość bboxów z vision — decision-point: zmierzono (self-test LLM-via-Read + realny API v6/v7/thinking, N=3), że bbox przez prompt jest nie do naprawienia (klastrowanie y2 + zawyżona szerokość wrodzone modelowi; v7 gorszy, thinking bez efektu). `PROMPT_VERSION` zostaje v6; pivot → S-43 identity-first.** — Archived 2026-06-09 → `context/archive/2026-06-07-bbox-quality-validation/`. Lesson: self-test „LLM-via-Read" jako darmowy proxy endpointu vision PRZED wydaniem na API; 2D-IoU to zła metryka dla wąskich grzbietów → metryki kierunkowe (xIoU/szer×/|Δy2|/centerHit); GT wymaga jednej zamrożonej konwencji.
- **S-44: utwardzenie flaky E2E — pełny `npm run test:e2e` przechodzi deterministycznie (3× zielony lokalnie, 179 passed). Wspólny moduł `tests/e2e/helpers/interactions.ts` (openUserMenu/createShelf/expectHoverBg); naprawa 3 flaków (account/shelves/dark-mode) + sweep lost-click (auth) i negative-wait (cost-panel); zero zmian w kodzie produkcyjnym; CI `retries:2` zostaje siatką na infra-blip, nie crutchem.** — Archived 2026-06-09 → `context/archive/2026-06-09-e2e-flake-stabilization/`. Lesson: flaki „zielone w izolacji, padają w pełnym przebiegu" to wyścig hydratacji wysp (lost-click) + ciasne timeouty na refetch + transientny odczyt computed-style; lek = bounded `toPass` walidowany side-effectem + `waitForResponse`, nie globalny retry.
- **S-43: Pivot pipeline vision na identity-first (rozpoznanie > lokalizacja)** — Archived 2026-06-12 → `context/archive/2026-06-09-vision-identity-first/`. Lesson: —.
- **S-26: użytkownik z flagą `is_admin=true` widzi dodatkowy link „Admin" w headerze; panel `/admin` zawiera: listę użytkowników (email, data rejestracji, liczba półek/książek, flagi), przełącznik `ai_enabled` per user (domyślnie false — blokuje wywołania vision/match), przycisk „Zaloguj jako" (impersonacja przez Supabase Admin API → `generateLink` + redirect), przycisk „Usuń konto" (books + shelf_entries → admin, półki → admin, potem `deleteUser`), akcję „Przenieś półkę" (zmiana `user_id` na innego usera). Migracja: nowe kolumny `profiles.is_admin bool default false` i `profiles.ai_enabled bool default false`.** — Archived 2026-06-12 → `context/archive/2026-06-12-admin-panel/`. Lesson: —.

- **S-50: użytkownik może ręcznie rozwiązać przez AI (Claude web_search, BYOK) detekcję bez kandydatów po wyczerpaniu strukturalnej kaskady (GB/OL/BN + word-level fallback)** — Zarchiwizowano 2026-07-13 → `context/archive/2026-07-13-ai-book-resolution-fallback/`. Lesson: post-archive eksperyment (2026-07-13) zmierzył empirycznie „jeden przebieg" (vision + `web_search` połączone w jednym wywołaniu per zdjęcie, zamiast dzisiejszej 2-fazowej kaskady) na realnym zdjęciu (6 książek, 1 trudny OCR-przypadek): **$0.4238 i 6 wyszukiwań ZAWSZE** (po jednym na każdą książkę, nawet te łatwe) vs **$0.0118 baseline + opcjonalnie ~$0.21 tylko dla faktycznie trudnego przypadku** w obecnej architekturze — czyli jeden przebieg jest ~2× droższy niż dzisiejszy worst-case i ~36× droższy niż typowy przypadek. Potwierdza pierwotną decyzję z opisu S-50 („NIE wpinać web search do promptu vision — koszt mnożyłby się drastycznie"): koszt jednego przebiegu skaluje się z **rozmiarem całej półki**, koszt obecnej architektury skaluje się wyłącznie z **liczbą faktycznie problematycznych detekcji**. Dodatkowo: przy JSON-array output (wiele książek naraz) model tym bardziej ignoruje instrukcję „TYLKO JSON" (długi tekst przed finalnym blokiem) i istniejący parser „ostatni blok `{...}`" (płaski obiekt) nie wystarczyłby — potrzebna byłaby ekstrakcja całej tablicy. Nie łączyć vision z web_search w jednym wywołaniu; jeśli potrzebna automatyzacja bez klikania, tańszy kierunek: auto-trigger istniejącego `resolve.ts` tylko dla `candidates.length === 0` po kaskadzie, nie łączenie z vision.
