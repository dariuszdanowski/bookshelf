...
- **E2E w CI na każdym PR** (od 2026-06-19, nielimitowane minuty Actions po przejściu repo na public): job `e2e` (Playwright + integracje RLS + bramka migracji) biega **automatycznie na każdym PR** obok `verify` (`paths-ignore` nadal pomija PR-y czysto dokumentacyjne). Dostępny też ręcznie (`workflow_dispatch` — Actions → „Run workflow" / `gh workflow run ci.yml --ref <branch>`), np. jako gate na main przed certyfikacją. Lokalny `npm run test:e2e` (na stacku WSL) pozostaje szybszą pętlą podczas pracy — odpalaj go zanim wypchniesz — ale CI jest teraz twardą bramką, a nie wyłącznie ręcznym fallbackiem. (Historycznie 2026-06-08→2026-06-19: `e2e` był `workflow_dispatch`-only dla oszczędności minut.)
...
### CI
- GitHub Actions (`ci.yml`): job `verify` (lint + typecheck + vitest + build) **oraz** job `e2e` (playwright + integracje RLS + migracje) — **OBA na każdym PR** (z `paths-ignore` docs/skille; od 2026-06-19, nielimitowane minuty Actions). `e2e` dostępny też ręcznie (`workflow_dispatch`) — zob. § Testy „E2E w CI na każdym PR". Deploy CF Workers w `deploy.yml` (`cloudflare/wrangler-action@v4`) na push do main; trigger `push:[main]` w `ci.yml` usunięty (post-merge waliduje deploy.yml + smoke).
